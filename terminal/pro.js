...
function renderAlerts(){
  ...
  els.alertsList.innerHTML=a.length?a.map(x=>`<div class="alert-row"><span><b>${esc(x.symbol)}</b><small>${x.direction==='above'?'выше':'ниже'} ${price(x.price,x.symbol)} ${esc(x.note||'')}</small></span><button data-alert-toggle="${esc(x.id)}">${x.active?'●':'○'}</button><button data-alert-delete="${esc(x.id)}">×</button></div>`).join(''):'<div class="card muted">Алертов нет.</div>';
}
...
async function prepareRestore(file){
  try{
    if(file.size>BACKUP_MAX_BYTES)throw new Error('Файл превышает безопасный лимит');const snapshot=parseBoundedJson(await file.text(),BACKUP_MAX_BYTES),summary=summarizeBackupSnapshot(snapshot);runtime.pendingRestore={snapshot,store:validateBackupSnapshot(snapshot)};
...
els.importWorkspace.onchange=async e=>{const f=e.target.files?.[0];if(!f)return;try{if(f.size>WORKSPACE_MAX_BYTES)throw new Error('Файл превышает безопасный лимит');const x=parseBoundedJson(await f.text(),WORKSPACE_MAX_BYTES);if(!x.ui||typeof x.ui!=='object'||Array.isArray(x.ui))throw new Error('Нет ui');store.ui=deepMerge(defaultStore().ui,x.ui);save();location.reload();}catch(err){toast('Ошибка импорта: '+err.message,'error');}finally{e.target.value='';}};
