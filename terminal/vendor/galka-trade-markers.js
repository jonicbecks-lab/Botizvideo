(() => {
  ...
  function saveStore(store) {
    try {
      ...
      // Chart history is best-effort and must never affect LIVE trading.
    }
  }
  ...
  function campaignNetPnl(campaign) {
    ...
    const fees = finite(campaign.cycleFees, 0);
    return gross - fees;
  }

  function upsert(store, coin, marker) {
    if (!VALID_COINS.has(coin) || !marker?.key || !(marker.time > 0)) return;
    store[coin] ||= {};
    const previous = store[coin][marker.key] || {};
    store[coin][marker.key] = { ...previous, ...marker, coin };
  }
  ...
  function compactMarkers(coin) {
    ...
    for (const rows of campaigns.values()) {
      ...
      for (const entry of entries) {
        result.push({
          ...entry,
          text: entry === deepest ? `L${entry.level}` : '',
        });
      }
      result.push({
        ...exit,
  ...
  function drawMarkerOverlay(target) {
    ...
    const rows = target.visibleWindow?.().rows || [];
    const range = target.currentPriceRange?.(rows);
    if (!geometry || !rows.length || !range || !(range.span > 0)) return;

    const firstTime = Number(rows[0].time);
    const lastTime = Number(rows.at(-1).time);
    const slot = geometry.plotWidth / rows.length;
    ...
    for (const marker of markers) {
      ...
      const index = nearestRowIndex(rows, time);
      if (index < 0) continue;
      const row = rows[index];
      const x = geometry.left + slot * index + slot / 2;
