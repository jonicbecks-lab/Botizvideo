...
class GalkaV2StrategyTests(unittest.TestCase):
    ...
    def test_normal_right_leg_uses_20_80_split_and_nine_entries(self):
        plan = build_v2_plan(60_000.0, 60_600.0, 5)
        self.assertEqual(len(plan.levels), 9)
        self.assertAlmostEqual(plan.upper_share, 0.20)
        self.assertAlmostEqual(plan.lower_share, 0.80)
        ...
        upper = sum(row.notional for row in plan.levels[:5])
        lower = sum(row.notional for row in plan.levels[5:])
        self.assertAlmostEqual(upper / (upper + lower), 0.20, delta=0.01)
        self.assertAlmostEqual(lower / (upper + lower), 0.80, delta=0.01)

        lower_rows = plan.levels[5:]
        lower_total = sum(row.notional for row in lower_rows)
        self.assertAlmostEqual(lower_rows[0].notional / lower_total, 0.50, delta=0.02)
        self.assertAlmostEqual(lower_rows[1].notional / lower_total, 0.125, delta=0.02)
        self.assertAlmostEqual(lower_rows[2].notional / lower_total, 0.1625, delta=0.02)
        self.assertAlmostEqual(lower_rows[3].notional / lower_total, 0.2125, delta=0.02)

    def test_tall_right_leg_reduces_upper_share_automatically(self):
        plan = build_v2_plan(60_000.0, 61_800.0, 5)
        self.assertLessEqual(plan.upper_share, 0.20)
        self.assertGreaterEqual(plan.upper_share, 0.10)
        self.assertGreaterEqual(plan.full_fill_net_at_galka, FULL_PLAN_MIN_NET_AT_GALKA_USD)

    def test_upper_tp_targets_one_percent_of_used_margin_net(self):
        # $200 entry notional at 10x uses $20 margin, so target net is $0.20.
        quantity = 2.0
        ...
        self.assertAlmostEqual(net, 0.20, places=8)
