...
if [[ ! -x "$PYTHON" ]]; then
  echo "Не найдено окружение GALKA V2. Сначала запусти setup." >&2
  exit 1
fi
if [[ ! -f "$CONFIG_FILE" || -L "$CONFIG_FILE" ]]; then
  echo "Не найден безопасный config GALKA V2: $CONFIG_FILE" >&2
  exit 1
fi
...
if [[ -f "$PID_FILE" ]]; then
  ...
  if [[ "$pid" =~ ^[0-9]+$ ]] && kill -0 "$pid" 2>/dev/null; then
    ...
    if [[ "$cmdline" == *"live.galka_v2_server_entry"* ]]; then
      echo "Перезапускаю GALKA V2 с LIVE-доступом..."
