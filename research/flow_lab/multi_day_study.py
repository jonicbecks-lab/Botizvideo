...
from .archive_study import _summarize, stream_buckets
...
def study_range(symbol: str, start: date, end: date, workdir: Path,
    ...
    for e in events:
        ...
        per_day.setdefault(day, {})[klass] = per_day.setdefault(day, {}).get(klass, 0) + 1
    ...
    return {
        ...
        "events_per_day_by_class": per_day,
        "horizon_seconds": {str(h): h * window_sec for h in cfg.horizons_buckets},
        "groups": _summarize(events, cfg.horizons_buckets),
        "interpretation": "fixed-parameter multi-day diagnostic; not a trading edge claim",
    }


def main() -> None:
    p = argparse.ArgumentParser(description="Fixed-parameter multi-day Binance flow event study")
    ...
    payload = {"study": "FLOW_BINANCE_FIXED_MULTIDAY", "results": results}
