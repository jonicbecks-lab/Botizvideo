...
# Correct the campaign risk cap: initial $10 tranche + at most 10 grid fills.
...
out=Path('research/super_model_output_cap11')/f'{symbol}_{year}'
out.mkdir(parents=True,exist_ok=True)
rows,m5=b.load_year(symbol,year)
replay=int(datetime(year,7,1,tzinfo=timezone.utc).timestamp()*1000)
results=[]; losses=[]
for mode in b.MODES:
    print('MODEL_CAP11',symbol,year,mode,flush=True)
    a,rs=(b.build_original(rows,replay) if mode=='ORIGINAL' else b.build_adaptive(rows,replay,mode))
    sim=b.Sim(a,m5,rs); metrics=sim.run(); metrics.update(symbol=symbol,year=year,mode=str(mode),risk_cap='11_tranches'); results.append(metrics)
    ...
    print('RESULT_CAP11',json.dumps(metrics,ensure_ascii=False),'MAX_TRANCHES',max_tr,flush=True)
...
print('DONE_CAP11',symbol,year,flush=True)
